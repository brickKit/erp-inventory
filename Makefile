IMAGE   := brickenterprise/erp-inventory
...
migrate-idempotent:  ## 同一份迁移连跑两次都必须成功（§13.3 铁律五）
	...
	@# erp_inventory_rw）：分区维护之类的 DDL 需要建表权限。
	@go build -o /tmp/erp-inventory-migrate-probe ./backend/cmd/migrate
	@/tmp/erp-inventory-migrate-probe up && /tmp/erp-inventory-migrate-probe up && echo "✓ 迁移幂等"

dag-check:  ## 强依赖图无环（§4.2）。物理命令枢纽，依赖为空，天然无环
	@if ! grep -qE '^\s*components:\s*\[\]' component.yaml; then \
	   echo "✗ erp-inventory 是物理命令枢纽，dependencies.components 必须为空（§2.6）"; exit 1; fi
	@echo "✓ 无强依赖，无环"
...
import-scan:  ## 铁律六：不许 import 任何其他组件仓库（§13.3）
	@bad="$$(go list -deps ./... 2>/dev/null | grep -E '^github.com/brickKit/' \
	         | grep -vE '^github.com/brickKit/(erp-inventory|be-sdk-go)(/|$$)' || true)"; \
	 if [ -n "$$bad" ]; then \
...
docs-check:  ## 四份文档结构检查（总纲 §4 SOP-D）
	@bash ../../../infra/scripts/docs-check.sh erp-inventory
