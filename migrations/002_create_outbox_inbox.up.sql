...
-- ⚠️ 实测踩坑（同 mdm-product 002 迁移）：建分区要求执行者是父表的
-- owner，迁移用管理凭据跑，建出来的表默认属于那个账号；分区维护后台
-- 任务运行时用 erp_inventory_rw（SET LOCAL ROLE 切换）建未来的分区，
-- 两者不是同一身份，必须显式把 owner 转过去。
ALTER TABLE event_outbox OWNER TO erp_inventory_rw;
...
-- 本组件消费 mdm.product.created.v1/.updated.v1 维护 product_tracking_snapshots
-- （设计计划 §4），这是第一次真的会走到这条路径。
CREATE UNIQUE INDEX event_inbox_idem ON event_inbox (idempotency_key, created_at);

-- 同上：分区维护后台任务要能给 event_inbox 建未来的分区。
ALTER TABLE event_inbox OWNER TO erp_inventory_rw;

-- 写命令的幂等表（命令侧，与事件消费侧的 event_inbox 分开）。Reserve/
-- CancelReservation/ConfirmIssue/Receive/Adjust 的 idempotency_key 落
-- 在这里去重；result_id 存该命令产生的业务标识（Reserve → reservation_id，
-- Receive/Adjust → movement_id），重放时直接把它原样返回，不重新跑一遍
-- 条件更新。
